...
def get_data_ASD(positiveGenePath, negativeGenePath, geneList):
    positive_gene=pd.read_csv(positiveGenePath,header=None)
    positive_gene=list(positive_gene[0].values)
    positive_gene=list(set(geneList)&set(positive_gene))

    negative_gene=pd.read_csv(negativeGenePath,header=None)
    negative_gene=list(negative_gene[0].values)
    negative_gene=list(set(geneList)&set(negative_gene))
    return  positive_gene,negative_gene

def fiveFoldCV(positive_gene, negative_gene, totaltimes, geneList, signalMatrix):
    AUROC = list()
    AUPRC = list()
...
_, signalMatrixPath, geneListPath, positiveGenePath, negativeGenePath,totaltimes = sys.argv
...
positive_gene,negative_gene = get_data_ASD(positiveGenePath, negativeGenePath, geneList)
AUROC,AUPRC = fiveFoldCV(positive_gene, negative_gene, totaltimes, geneList, signalMatrix)
